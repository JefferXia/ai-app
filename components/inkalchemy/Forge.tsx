...
export const Forge: React.FC<ForgeProps> = ({
  project,
  onComplete,
  onReset,
  onToggleFavorite,
  ...
  const [selectedModel, setSelectedModel] = useState<string>(
    project.selectedModel || 'google/gemini-2.5-flash'
  );
  ...
  // Sync selectedModel with project state
  useEffect(() => {
    if (project.selectedModel && project.selectedModel !== selectedModel) {
  ...
  const handleBrainstorm = async () => {
    ...
    try {
      const newIdeasRaw = await generateBrainstormIdeas(
        topic,
        round,
        existingTitles,
        selectedModel
      ...
      const ideasArray = Array.isArray(newIdeasRaw)
        ? newIdeasRaw
        : Array.isArray(newIdeasRaw.ideas)
          ? newIdeasRaw.ideas
          : [];

      const newIdeas: Idea[] = ideasArray.map((raw: any, index: number) => ({
        id: `${topic}-${round}-${Date.now()}-${index}-${Math.random().toString(36).substring(2, 9)}`,
        title: raw.title,
      ...
      if (errorMsg.includes('403') || errorMsg.includes('forbidden')) {
        alert(
          `模型 ${modelName} 暂时不可用，请尝试其他模型（推荐使用带⭐标识的模型）`
        );
      } else if (errorMsg.includes('quota') || errorMsg.includes('billing')) {
        alert(`模型 ${modelName} 配额不足，请尝试其他模型或稍后重试`);
      } else {
        alert('生成想法失败，请重试');
      }
    } finally {
  ...
  const isFavorite = (ideaId: string) => {
    return project.favorites.some(
      (f) => f.content.title === ideas.find((i) => i.id === ideaId)?.title
    );
  };
    ...
    <div className="p-8 max-w-5xl mx-auto h-full overflow-y-auto custom-scrollbar">
      <div className="mb-8">
        ...
        <p className="text-stone-400">
          输入一个核心词，我们将通过&ldquo;3+3+3&ldquo;阶梯脑暴，提炼出具有商业价值的选题。
        </p>
        {!isAuth && (
        ...
        <div className="flex flex-col gap-4 max-w-xl mx-auto mt-20">
          <div className="flex items-center justify-between">
            ...
            <button
              ...
              className="text-xs flex items-center gap-1.5 px-3 py-1.5 text-stone-400 hover:text-amber-500 border border-stone-700 hover:border-amber-700 rounded transition-colors"
            >
              <Settings size={14} />
              模型:{' '}
              {AVAILABLE_MODELS.find((m) => m.id === selectedModel)?.name ||
          ...
          {showModelSelector && (
            <div className="grid grid-cols-1 gap-2 p-4 bg-stone-900/50 border border-stone-800 rounded-lg">
              <div className="text-xs text-stone-500 mb-2">
                选择AI模型 (推荐使用带⭐标识的模型，性能稳定)
              </div>
                ...
                >
                  <div>
                    <div className="flex items-center gap-2">
                      ...
                      {model.recommended && (
                        <span className="text-xs px-2 py-0.5 rounded bg-amber-900/30 text-amber-400 border border-amber-700/50">
                          ⭐ 推荐
                        </span>
                      )}
                    </div>
                  ...
                  <div className="text-xs text-stone-600">{model.provider}</div>
                </button>
              ))}
            </div>
          )}
      ...
      {ideas.length === 0 && loading && (
        <div className="flex flex-col items-center justify-center h-[50vh] w-full border border-stone-800/50 rounded-lg bg-stone-900/20">
          <div className="relative mb-6">
            ...
            <Flame
              size={64}
              className="relative z-10 text-amber-500 animate-bounce"
            />
          </div>
          <h2 className="text-2xl font-serif font-bold text-stone-200 tracking-widest animate-pulse mb-2">
            AI 炼金中...
          </h2>
          <p className="text-stone-500 font-mono text-sm">
            正在提炼商业价值 (Calculating Market Fit)...
          </p>
        </div>
      )}
      ...
      {ideas.length > 0 && (
        <div className="space-y-8 pb-20">
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
            <div className="col-span-1 md:col-span-3 mb-4 p-4 bg-blue-900/20 border border-blue-900/30 rounded-lg">
              <div className="flex items-center gap-2 text-blue-400 text-sm">
                <Sparkles size={16} />
                <span>点击任意想法卡片选择它，然后进入下一阶段</span>
              </div>
            </div>
            {ideas.map((idea) => {
                ...
                <div
                  ...
                  className={`group p-6 rounded-lg border transition-all duration-300 relative overflow-hidden flex flex-col ${
                    isSelected
                      ...
                      : 'bg-stone-900/50 border-stone-800 hover:border-amber-700 hover:bg-stone-900'
                  } ${isAuth ? 'cursor-default' : 'opacity-50'}`}
                >
                  <div className="flex justify-between items-start mb-3">
                    <span
                      className={`text-xs px-2 py-1 rounded border ${
                        idea.type === 'contrarian'
                    ...
                    <button
                      ...
                      className={`w-full py-2 px-4 rounded font-medium text-sm transition-colors ${
                        isSelected
                          ...
                          : isAuth
                            ? 'bg-stone-800 text-stone-300 hover:bg-amber-700 hover:text-white'
                            : 'bg-stone-900 text-stone-600 cursor-not-allowed'
                      }`}
                    >
                      {isSelected ? '✓ 已选择' : '选择此想法进入下一阶段'}
                    </button>
                  </div>

                  <div
                    className={`absolute top-0 right-0 p-2 transition-opacity ${isSelected ? 'opacity-100' : 'opacity-0 group-hover:opacity-100'}`}
                  >
                    {isSelected && (
